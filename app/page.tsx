...
export default function Home() {
  ...
  const [fxUpdatedAt, setFxUpdatedAt] = useState<string>("기본값 표시 중");
  ...
  useEffect(() => {
    async function fetchFx() {
      try {
        const res = await fetch("https://api.frankfurter.dev/v2/rates?base=AUD&quotes=KRW", {
          cache: "no-store",
        });
        ...
        if (rate) {
          setAudToKrw(rate);
          setFxUpdatedAt(new Date().toLocaleTimeString("ko-KR"));
        }
      } catch (error) {
  ...
  return (
    <main className="min-h-screen px-6 py-8 text-slate-800 md:px-10">
      <div className="mx-auto max-w-7xl">
        <header className="relative mb-8 overflow-hidden rounded-[36px] border border-white/60 bg-gradient-to-br from-sky-100 via-white to-indigo-100 p-8 shadow-sm">
            ...
            <div className="mt-4 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-2xl bg-slate-900 px-4 py-3 text-white shadow-sm">
                  ...
                  <p className="mt-1 text-xs text-slate-300">{fxUpdatedAt}</p>
                </div>
              </div>
            </div>
          </div>
